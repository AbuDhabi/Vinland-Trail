...
int high_seas() {
    ...
    while (!done) {
        ...
        if (!PAUSED) {
            if (PACE == 0) {
                DISTANCE += (int)(36 + rand()%6 - rand()%6 + skill_mod/2)*(INVENTORY.boat_status/100.0);
                for (int i=0;i<5;i++) {
                    change_health(i,1);
                }
            } else if (PACE == 1) {
                DISTANCE += (int)(72 + rand()%12 - rand()%12 + skill_mod)*(INVENTORY.boat_status/100.0); // from SRD, 3mi/hr
            } else if (PACE == 2) {
                DISTANCE += (int)(108 + rand()%18 - rand()%18 + skill_mod*2)*(INVENTORY.boat_status/100.0);
                for (int i=0;i<5;i++) {
                    change_health(i,-1);
                }
            }
            if (RATIONS == 0) {
                ...
                for (int i=0;i<5;i++) {
                    if (PARTY[i].health > 0) { INVENTORY.food--; } // normal
                    if (PARTY[i].health > 0) { INVENTORY.food--; } // normal
                }
                INVENTORY.food -= INVENTORY.settlers;
                INVENTORY.food -= INVENTORY.settlers;
                ...
                for (int i=0;i<5;i++) {
                    if (PARTY[i].health > 0) { INVENTORY.food--; } // sir DOUBLE FAGGOT
                    if (PARTY[i].health > 0) { INVENTORY.food--; } 
                    if (PARTY[i].health > 0) { INVENTORY.food--; } // sir DOUBLE FAGGOT
                    if (PARTY[i].health > 0) { INVENTORY.food--; } 
                    change_health(i,1);
                }
        ...
        // sanity checks
        if (INVENTORY.food < 1) { RATIONS = 0; } // why can't I feed all these mouths?
        ...
        if (PAUSED) 
            ...
            SDL_Delay(500);  // TODO: adjust this properly, so it's not too slow and not too fast
    }
